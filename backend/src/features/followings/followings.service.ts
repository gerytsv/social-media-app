...
export class FollowingsService {
  ...
  public async followUser(requestUserId: string, userId: string) {
    ...
    const newrequestUserFollowedUsers = [
      ...requestUserFollowedUsers,
      userToBeFollowed,
    ];
    requestUser.followed = Promise.resolve(newrequestUserFollowedUsers);
    await this.userRepository.save(requestUser);
    const followedSuccesfully = await this.userRepository.save(
      userToBeFollowed,
    );
    if (followedSuccesfully) {
      const adminHistoryEntity = this.historyRepository.create();
      adminHistoryEntity.content = `User ${
        requestUser.username
      } has followed user ${userToBeFollowed.username}`;
  ...
  public async unFollowUser(requestUserId: string, userId: string) {
    ...
    const requestUserFollowedUsers = await requestUser.followed;
    const indexOfFollower = followersOfuserToBeUnFollowed.findIndex(
      user => user.id == requestUserId,
    );
    followersOfuserToBeUnFollowed.splice(indexOfFollower, 1);
    const indexOfFollowed = requestUserFollowedUsers.findIndex(
      user => user.id == userId,
    );
    ...
    userToBeUnFollowed.followers = Promise.resolve(
      followersOfuserToBeUnFollowed,
    );
