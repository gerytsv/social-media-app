...
const getDtoMetadata = (target: new () => any): KeyOption[] => {
  ...
  if (!metadata) {
    throw new Error(
      `Expected class ${target.name} to have published properties!`,
    );
  }
...
export const transformToDTO = <T extends new () => any>(
  target: T,
  original: any,
): T => {
  ...
  const result = metadata.reduce(
    (constructed: any, option) =>
      option.dto !== null
        ? ((constructed[option.key] = transformToDTO(
            option.dto,
            original[`__${option.key}__`] || original[option.key],
          )),
          constructed)
        : Object.keys(original).includes(`__${option.key}__`)
        ? ((constructed[option.key] = original[`__${option.key}__`]),
          constructed)
